...
with sync_playwright() as p:
    ...
    search_box.focus()
    search_box.type('Digital Citizenship', delay=100)
    time.sleep(5)
    page.locator('.dropdown-menu.active >> text="Digital Citizenship"').click(force=True)
    time.sleep(5)   
    ...
    with open("Alltechjobs8.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Job Title", "Company/Organization", "Location", "Job Link"])
        writer.writerows(jobs)

    print(f"Get Alltechjobs8.csv , {len(jobs)} jobs。")
